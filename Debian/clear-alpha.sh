...
if [ -n "$kernel_packages" ]; then
    ...
    apt-get purge --assume-yes $kernel_packages > /dev/null 2>&1
    if [ $? -eq 0 ]; then
        log_success "旧内核删除成功。"
...
if [ -n "$orphaned_libs" ]; then
    ...
    echo "$orphaned_libs"
    apt-get -y remove --purge $orphaned_libs > /dev/null 2>&1
    if [ $? -eq 0 ]; then
        log_success "孤立库清理成功。"
    else
        log_error "孤立库清理失败。"
...
if [ -n "$orphaned_data" ]; then
    ...
    echo "$orphaned_data"
    apt-get -y remove --purge $orphaned_data > /dev/null 2>&1
    if [ $? -eq 0 ]; then
...
if command -v docker &> /dev/null; then
    ...
    read -p "${YELLOW}警告：此操作将删除所有未使用的 Docker 镜像、容器、网络和卷。是否继续？ (y/N)${NC} " confirm_docker_prune
